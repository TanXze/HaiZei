...
if [[ ${Disk_useper}<80 ]]; then
    ...
    Disk_alarm="warning"
    echo "$Time $User Disk $Disk_alarm Disk usage $Disk_useper" >> ~/HaiZei/Monitoring_system/shell/logfile/warning.log
...
if [[ ${CPU_temperature}<50 ]]; then 
    ...
    CPU_alarm="warning"
    echo "$Time $User CPU $CPU_alarm CPU temperature $CPU_temperature" >> ~/HaiZei/Monitoring_system/shell/logfile/warning.log
fi
...
if [[ ${MEM_useper}<70 ]]; then
    ...
    MEM_alarm="warning"
    echo "$Time $User MEM $MEM_alarm MEM usage $MEM_useper" >> ~/HaiZei/Monitoring_system/shell/logfile/warning.log
fi
